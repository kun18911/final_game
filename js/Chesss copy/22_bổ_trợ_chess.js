...
function crush_Xong_Rơi(xx) {
    if (yyy) {cộngĐiểmSauCrush(0)}; create4x_and3x();
    setTimeout(function () {
...
//     var isSpecial = classNames.some(x => mười_Tám_Class.includes(x));
//     var isBassic = classNames2.some(x => sáu_Class_Thường.includes(x));
...
// 6.
function test_Có_Tạo_4x_Và_5x_ko() {
    for (var i = 0; i < listOfArrays.length; i++) {
        if (listOfArrays[i].length == 4) {mảng_4_Ô.push(listOfArrays[i])}
        if (listOfArrays[i].length >= 5) {mảng_5_Ô.push(listOfArrays[i])}
    }
